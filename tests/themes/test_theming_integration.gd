...
func test_formatter_theme_setting() -> void:
	var custom_theme: LogTheme = LogTheme.new()
	custom_theme.error_color = Color.MAGENTA
	custom_theme.theme_name = "Test Theme"
	
	formatter.set_theme(custom_theme)
	var retrieved_theme: LogTheme = formatter.get_theme()
...
func test_formatter_colored_message_formatting() -> void:
	var custom_theme: LogTheme = LogTheme.new()
	...
	var error_message: String = formatter.format_message_with_colors("TestLogger", LogLevel.Level.ERROR, "Error text")
	var info_message: String = formatter.format_message_with_colors("TestLogger", LogLevel.Level.INFO, "Info text")
...
func test_formatter_timestamp_coloring() -> void:
	var custom_theme: LogTheme = LogTheme.new()
	custom_theme.timestamp_color = Color.BLUE
	formatter.set_theme(custom_theme)
	formatter.set_timestamps_enabled(true)
	
	var message: String = formatter.format_message_with_colors("Test", LogLevel.Level.INFO, "Message")
...
func test_output_theme_setting() -> void:
	var custom_theme: LogTheme = LogTheme.new()
	custom_theme.theme_name = "Output Test Theme"
	
	output.set_theme(custom_theme)
	var retrieved_theme: LogTheme = output.get_theme()
	
	assert_object(retrieved_theme).is_equal(custom_theme)
	assert_str(retrieved_theme.theme_name).is_equal("Output Test Theme")

func test_output_theme_propagation_to_formatter() -> void:
	var custom_theme: LogTheme = LogTheme.new()
	custom_theme.warn_color = Color.PURPLE
	
	output.set_theme(custom_theme)
	var formatter_theme: LogTheme = output.formatter.get_theme()
...
func test_manager_theme_initialization() -> void:
	var default_theme: LogTheme = manager.get_current_theme()
	
	assert_object(default_theme).is_not_null()
	assert_str(default_theme.theme_name).is_equal("Default")

func test_manager_set_theme() -> void:
	var fallout_theme: LogTheme = manager.get_theme_by_name("Fallout")
	assert_object(fallout_theme).is_not_null()
	
	manager.set_theme(fallout_theme)
	var current_theme: LogTheme = manager.get_current_theme()
	
	assert_object(current_theme).is_equal(fallout_theme)
	assert_str(current_theme.theme_name).is_equal("Fallout")

func test_manager_set_theme_by_name() -> void:
	var success: bool = manager.set_theme_by_name("Minimal")
	assert_bool(success).is_true()
	
	var current_theme: LogTheme = manager.get_current_theme()
	assert_str(current_theme.theme_name).is_equal("Minimal")

func test_manager_set_invalid_theme_by_name() -> void:
	var success: bool = manager.set_theme_by_name("NonExistentTheme")
...
func test_manager_custom_theme_management() -> void:
	var custom_theme: LogTheme = LogTheme.new()
	...
	var available_themes: Array[String] = manager.get_available_themes()
	assert_array(available_themes).contains(["MyCustomTheme"])
	
	var retrieved_theme: LogTheme = manager.get_theme_by_name("MyCustomTheme")
	assert_object(retrieved_theme).is_equal(custom_theme)
	assert_object(retrieved_theme.error_color).is_equal(Color.ORANGE)

func test_manager_get_available_themes() -> void:
	var themes: Array[String] = manager.get_available_themes()
...
func test_logger_node_theme_setting() -> void:
	var minimal_theme: LogTheme = logger_node.get_theme_by_name("Minimal")
	logger_node.set_theme(minimal_theme)
	
	var current_theme: LogTheme = logger_node.get_current_theme()
	assert_object(current_theme).is_equal(minimal_theme)
	assert_str(current_theme.theme_name).is_equal("Minimal")

func test_logger_node_set_theme_by_name() -> void:
	var success: bool = logger_node.set_theme_by_name("Whiteout")
	assert_bool(success).is_true()
	
	var current_theme: LogTheme = logger_node.get_current_theme()
	assert_str(current_theme.theme_name).is_equal("Whiteout")

func test_logger_node_get_available_themes() -> void:
...
func test_logger_node_custom_theme_addition() -> void:
	var custom_theme: LogTheme = LogTheme.new()
	...
	var retrieved: LogTheme = logger_node.get_theme_by_name("NodeCustomTheme")
	assert_object(retrieved).is_equal(custom_theme)

# Test theme effects on actual logging output
func test_themed_logging_to_file() -> void:
...
func test_theme_affects_named_loggers() -> void:
	var custom_theme: LogTheme = LogTheme.new()
	custom_theme.debug_color = Color.CYAN
	custom_theme.theme_name = "NamedLoggerTheme"
	
	manager.set_theme(custom_theme)
	var named_logger: LoggerInstance = manager.get_logger("ThemedLogger", LogLevel.Level.DEBUG)
...
func test_default_theme_properties() -> void:
	var default_theme: LogTheme = manager.get_theme_by_name("Default")
	
	assert_str(default_theme.theme_name).is_equal("Default")
	assert_object(default_theme.error_color).is_equal(Color.RED)
	assert_object(default_theme.warn_color).is_equal(Color.YELLOW)

func test_minimal_theme_properties() -> void:
	var minimal_theme: LogTheme = manager.get_theme_by_name("Minimal")
...
func test_whiteout_theme_properties() -> void:
	var whiteout_theme: LogTheme = manager.get_theme_by_name("Whiteout")
...
func test_fallout_theme_properties() -> void:
	var fallout_theme: LogTheme = manager.get_theme_by_name("Fallout")
...
func test_multiple_loggers_same_theme() -> void:
	var custom_theme: LogTheme = LogTheme.new()
	custom_theme.theme_name = "SharedTheme"
	manager.set_theme(custom_theme)
	
	var logger1: LoggerInstance = manager.get_logger("Logger1")
	var logger2: LoggerInstance = manager.get_logger("Logger2")
...
# Test error cases
func test_null_theme_handling() -> void:
	# This should not crash the system
	var original_theme: LogTheme = manager.get_current_theme()
