...
func test_default_colors() -> void:
	var expected_gray: Color = Color(0.804, 0.812, 0.824)
...
func test_get_color_for_level_all_levels() -> void:
...
func test_get_color_for_level_invalid() -> void:
	# Test with invalid level (should return WHITE as fallback)
	@warning_ignore("int_as_enum_without_match")
	var invalid_level: LogLevel.Level = 999 as LogLevel.Level
	assert_object(theme.get_color_for_level(invalid_level)).is_equal(Color.WHITE)

# Test custom color assignment
func test_custom_color_assignment() -> void:
	var custom_red: Color = Color(0.8, 0.2, 0.2)
	var custom_blue: Color = Color(0.2, 0.2, 0.8)
...
func test_default_themes_exist() -> void:
...
func test_default_themes_are_log_themes() -> void:
	for theme_name: String in LogTheme.default_themes.keys():
		var loaded_theme: LogTheme = LogTheme.default_themes[theme_name]
		assert_object(loaded_theme).is_instanceof(LogTheme)
		assert_str(loaded_theme.theme_name).is_equal(theme_name)

# Test color consistency
func test_color_consistency() -> void:
	# Colors should remain consistent across multiple calls
	var initial_error_color: Color = theme.get_color_for_level(LogLevel.Level.ERROR)
	var second_error_color: Color = theme.get_color_for_level(LogLevel.Level.ERROR)
	
	assert_object(initial_error_color).is_equal(second_error_color)

# Test edge cases with extreme color values
func test_extreme_color_values() -> void:
...
func test_color_isolation() -> void:
	var original_info_color: Color = theme.info_color
