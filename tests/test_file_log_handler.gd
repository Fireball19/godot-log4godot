...
# Test initialization
func test_initialization_default_path() -> void:
	var default_handler: FileLogHandler = FileLogHandler.new()
	assert_str(default_handler.file_path).is_equal("user://game.log")
	assert_bool(default_handler.is_enabled).is_false()

func test_initialization_custom_path() -> void:
...
func test_set_enabled_creates_file() -> void:
	handler.set_enabled(true)
	assert_bool(FileAccess.file_exists(test_file_path)).is_true()

# Test file path setting
func test_set_file_path() -> void:
	var new_path: String = "user://new_test_log.log"
...
func test_set_file_path_when_enabled() -> void:
	var new_path: String = "user://new_test_log.log"
	handler.set_enabled(true)
	handler.set_file_path(new_path)
...
# Test log writing
func test_write_log_when_disabled() -> void:
...
func test_write_log_when_enabled() -> void:
	handler.set_enabled(true)
	handler.write_log("Test message")
...
func test_write_multiple_logs() -> void:
	handler.set_enabled(true)
...
func test_write_log_appends() -> void:
	handler.set_enabled(true)
	handler.write_log("First message")
	
	# Create new handler instance to simulate restart
	var handler2: FileLogHandler = FileLogHandler.new(test_file_path)
...
# Test log clearing
func test_clear_log_when_disabled() -> void:
	handler.clear_log_file()
	assert_bool(FileAccess.file_exists(test_file_path)).is_false()

func test_clear_log_when_enabled() -> void:
	handler.set_enabled(true)
...
func test_session_start_message() -> void:
	handler.set_enabled(true)
...
func test_session_start_message_format() -> void:
	handler.set_enabled(true)
...
func test_write_log_empty_message() -> void:
	handler.set_enabled(true)
	handler.write_log("")
...
func test_write_log_special_characters() -> void:
	handler.set_enabled(true)
	var special_message: String = "Test with special chars: @#$%^&*()[]{}|\\:;\"'<>?,./`~"
...
func test_write_log_unicode() -> void:
	handler.set_enabled(true)
	var unicode_message: String = "Unicode test: 你好世界 🎮 γειά σας"
...
func test_multiple_writes_sequence() -> void:
	handler.set_enabled(true)
