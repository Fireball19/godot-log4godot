...
func test_get_logger_creates_new() -> void:
	var logger: LoggerInstance = manager.get_logger("NetworkLogger")
...
func test_get_logger_returns_existing() -> void:
	var logger1: LoggerInstance = manager.get_logger("TestLogger")
	var logger2: LoggerInstance = manager.get_logger("TestLogger")
	
	assert_object(logger1).is_equal(logger2)

func test_get_logger_with_custom_level() -> void:
	var logger: LoggerInstance = manager.get_logger("CustomLogger", LogLevel.Level.ERROR)
	assert_int(logger.log_level).is_equal(LogLevel.Level.ERROR)

func test_get_logger_with_default_level() -> void:
	manager.set_global_level(LogLevel.Level.WARN)
	var logger: LoggerInstance = manager.get_logger("DefaultLogger")
	assert_int(logger.log_level).is_equal(LogLevel.Level.WARN)

func test_remove_logger_existing() -> void:
	manager.get_logger("ToRemove")
	assert_bool(manager.named_loggers.has("ToRemove")).is_true()
	
	var removed: bool = manager.remove_logger("ToRemove")
	assert_bool(removed).is_true()
	assert_bool(manager.named_loggers.has("ToRemove")).is_false()

func test_remove_logger_non_existing() -> void:
	var removed: bool = manager.remove_logger("NonExistent")
	assert_bool(removed).is_false()

func test_list_loggers_empty() -> void:
	var loggers: Array[String] = manager.list_loggers()
	assert_int(loggers.size()).is_equal(0)

func test_list_loggers_with_loggers() -> void:
...
func test_list_loggers_after_removal() -> void:
	manager.get_logger("Logger1")
	manager.get_logger("Logger2")
...
func test_global_level_provider() -> void:
	var test_logger: LoggerInstance = manager.get_logger("TestLogger", LogLevel.Level.DEBUG)
...
func test_multiple_loggers_independent() -> void:
	var logger1: LoggerInstance = manager.get_logger("Logger1", LogLevel.Level.DEBUG)
	var logger2: LoggerInstance = manager.get_logger("Logger2", LogLevel.Level.ERROR)
...
func test_multiple_loggers_share_output() -> void:
	manager.set_file_logging_enabled(true, test_file_path)
	
	var logger1: LoggerInstance = manager.get_logger("Logger1", LogLevel.Level.INFO)
	var logger2: LoggerInstance = manager.get_logger("Logger2", LogLevel.Level.INFO)
...
func test_configuration_affects_all_loggers() -> void:
	var logger1: LoggerInstance = manager.get_logger("Logger1", LogLevel.Level.INFO)
	var logger2: LoggerInstance = manager.get_logger("Logger2", LogLevel.Level.INFO)
...
func test_logger_name_edge_cases() -> void:
	# Test empty name
	var empty_logger: LoggerInstance = manager.get_logger("")
	assert_str(empty_logger.name).is_equal("")
	
	# Test name with spaces
	var space_logger: LoggerInstance = manager.get_logger("Logger With Spaces")
	assert_str(space_logger.name).is_equal("Logger With Spaces")
	
	# Test special characters
	var special_logger: LoggerInstance = manager.get_logger("Logger@#$%")
	assert_str(special_logger.name).is_equal("Logger@#$%")

func test_global_level_extreme_values() -> void:
...
func test_rapid_logger_creation() -> void:
	var logger_names: Array[String] = []
	
	# Rapidly create and access loggers
	for i: int in range(10):
		var logger_name: String = "RapidLogger" + str(i)
		logger_names.append(logger_name)
		var logger: LoggerInstance = manager.get_logger(logger_name)
		logger.info("Message from " + logger_name)
	
	assert_int(manager.list_loggers().size()).is_equal(10)
	
	# Verify all names are present
	var listed_loggers: Array[String] = manager.list_loggers()
	for logger_name: String in logger_names:
		assert_array(listed_loggers).contains([logger_name])

func test_logger_level_inheritance() -> void:
	# Test that new loggers inherit current global level
	manager.set_global_level(LogLevel.Level.WARN)
	var logger1: LoggerInstance = manager.get_logger("InheritLogger1")
	
	manager.set_global_level(LogLevel.Level.ERROR)
	var logger2: LoggerInstance = manager.get_logger("InheritLogger2")
